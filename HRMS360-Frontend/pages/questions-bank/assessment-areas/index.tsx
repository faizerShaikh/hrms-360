...
export const getServerSideProps = async (ctx: NextPageContext) => {
  setApiHeaders(ctx);
  const res = await serverAPI.get("/setting/area-assessment", {
    params: {
      page: 0,
      limit: 25,
    },
...

  {
    ...
    renderCell: ({ row }: GridRenderCellParams) => (
      <div className="flex items-center">
        <AssessmentAreaDialog isUpdate data={row} />
...
const AssessmentAreaConfiguration: BaseProps<AssessmentAreaInterface[]> = ({
  ...
  return (
    <>
      <PageHeader title="Assessment Areas Configuration" />
      <DataGrid
        columns={columns}
        rows={data}
        url="/setting/area-assessment"
        addButton={<AssessmentAreaDialog />}
      />
    </>
  );
};
